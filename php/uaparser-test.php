...
if (php_sapi_name() == "cli") {
	...
	print "\n\nrunning contributed tests...";

	$dirs = scandir($basePath."../test_resources/contributed/");
	foreach($dirs as $dir) {
		
		if (($dir != '.') && ($dir != '..')) {
			
			$info = Spyc::YAMLLoad($basePath."../test_resources/contributed/".$dir."/info.yaml");
			print "\n\nrunning uaparser.php against ".$info['name']." contributed by ".$info['author']." (".$info['username'].")...\n";
			
			$test_files = array("test_user_agent_parser","test_user_agent_parser_os","test_device");
			foreach($test_files as $test_file) {
				$data = Spyc::YAMLLoad($basePath."../test_resources/contributed/".$dir."/".$test_file.".yaml");
			    foreach($data["test_cases"] as $test_case) {
			        if (!isset($test_case["js_ua"])) {
						$bool   = true;
			            $result = $parser->parse($test_case["user_agent_string"]);
